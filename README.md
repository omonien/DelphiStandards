...
For developers working in WSL (Windows Subsystem for Linux), the repository includes a Bash wrapper [DelphiBuildDPROJ.sh](DelphiBuildDPROJ.sh) that bridges WSL to the Windows Delphi toolchain via `powershell.exe`. It accepts the same parameters as the PowerShell script and translates WSL paths to Windows paths automatically.
...
- `powershell.exe` accessible in `PATH` from within WSL (default on standard WSL installations)
- Embarcadero Delphi installed on the Windows host
